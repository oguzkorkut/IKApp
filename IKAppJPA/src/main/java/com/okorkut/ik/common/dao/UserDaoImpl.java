...
public class UserDaoImpl extends BaseDao  implements UserDao{
	...
	public User getUserByEmailAndPassword(String email, String password) throws Exception {
		...
		try {
			em = openConnection();
			user = (User) em.createNamedQuery("User.findGetUserByEmailAndPassword").setParameter("email", email).setParameter("password", password).getSingleResult();
			
		}catch (NoResultException nre) {
			logger.error(nre,nre);
		}catch (Exception e) {
			logger.error(e,e);
		}finally {
			closeConnection(em);
